...
$GLTE = $_POST['GLTE']  == '' 	? 'NULL' : $_POST['GLTE'];
$GVTE = $_POST['GVTE']  == '' 	? 'NULL' : $_POST['GVTE'];
$PL	  = $_POST['PL']    == '' 	? 'NULL' : $_POST['PL'];
$PV	  = $_POST['PV']    == '' 	? 'NULL' : $_POST['PV'];

$Resultados = array(	'Match' => $Match, 
						'GolesLocal' => $GL, 		'GolesVisita' => $GV, 
						'GolesLocalPT' => $GLPT, 	'GolesVisitaPT' => $GVPT, 
						'MinutoPrimerGol' => $MPG, 	'JugPrimerGol' => $JPG,
						'GolesLocalTE' => $GLTE, 	'GolesVisitaTE' => $GVTE,
						'PenalesLocal' => $PL, 		'PenalesVisita' => $PV);
						
if(!isset($_SESSION)) { session_start(); }

if ($_SESSION['id_user'] != '') {
	if (!mysqli_connect_errno()) { echo UpdateResults($conn, $Resultados); }
