-- Expose per-rule side-bet hit counts on the leaderboard (prórroga, primer gol).
...
match_agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE extra_time_points > 0) AS extra_time_hits,
    COUNT(*) FILTER (WHERE first_goal_points > 0) AS first_goal_hits,
...
owner_points AS (
  SELECT
    ...
    COALESCE(SUM(ma.extra_time_hits), 0)::bigint AS extra_time_hits,
    COALESCE(SUM(ma.first_goal_hits), 0)::bigint AS first_goal_hits,
...
totals AS (
  SELECT
    ...
    COALESCE(op.extra_time_hits, 0) AS extra_time_hits,
    COALESCE(op.first_goal_hits, 0) AS first_goal_hits,
...
COMMENT ON VIEW public.v_pool_rankings IS
  'Polla leaderboard: one row per owner with at least one pool. Includes extra_time_hits and first_goal_hits for side-bet breakdown.';
