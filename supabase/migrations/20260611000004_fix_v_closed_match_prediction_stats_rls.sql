-- Remote had security_invoker=on (default in PG15+), so the view ran as the caller and
-- predictions RLS (pool-owner only) limited aggregates to each user's own pool.
-- Recreate with security_invoker=false so all authenticated users see pool-wide counts
-- without exposing individual prediction rows.
